...
function MenuTab() {
  ...
  return (
    <div>
      ...
      {selectedLocation && (
        <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm mb-4 text-sm text-gray-600 space-y-2">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            <a href={adminApi.getMenuQrUrl(selectedSlug)} target="_blank" rel="noreferrer"
              className="px-4 py-2 rounded-xl bg-coffee-600 text-white text-xs font-semibold hover:bg-coffee-700 transition-colors">
              QR SVG
            </a>
            <a href={adminApi.getPrintMenuUrl(selectedSlug)} target="_blank" rel="noreferrer"
              className="px-4 py-2 rounded-xl bg-gray-900 text-white text-xs font-semibold hover:bg-black transition-colors">
              Друк меню
            </a>
          </div>
        </div>
              ...
              {isManualMenu && (
                <div className="flex gap-2 flex-1 justify-end">
                  <input value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} placeholder="Нова категорія"
                    className="flex-1 max-w-[240px] border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white" />
          ...
          {selectedCat && (
            <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
              ...
              {isManualMenu && (creatingProduct || editingProductId !== null) && (
                <div className="mb-3 rounded-2xl border border-gray-100 bg-gray-50 p-4 space-y-3">
                  <div className="text-sm font-medium text-gray-700">{editingProductId ? 'Редагування позиції' : 'Нова позиція'}</div>
                  ...
                  <input value={productDraft.category} onChange={(e) => setProductDraft((current) => ({ ...current, category: e.target.value }))}
                    list="cpanel-menu-categories" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white" placeholder="Категорія" />
                  <textarea value={productDraft.description} onChange={(e) => setProductDraft((current) => ({ ...current, description: e.target.value }))}
                    className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white" rows={3} placeholder="Опис" />
                  ...
                  <div className="grid grid-cols-4 gap-3">
                    ...
                    <label className="col-span-2 flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700">
                  ...
                  <div className="flex gap-2">
                    <button onClick={saveProductChanges} disabled={saving}
                      className="px-4 py-2 rounded-xl bg-coffee-600 text-white text-sm font-semibold disabled:opacity-50">
                      {editingProductId ? 'Зберегти зміни' : 'Створити позицію'}
                    </button>
                    <button onClick={cancelEditProduct} disabled={saving}
                      className="px-4 py-2 rounded-xl bg-white border border-gray-200 text-sm font-semibold text-gray-700 disabled:opacity-50">
                      Скасувати
                    </button>
                  </div>
                  <datalist id="cpanel-menu-categories">
                ...
                {catProducts.map((p, idx) => (
                  <div key={p.id} className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${
                    ...
                    <div className="flex items-center gap-2">
                      {isManualMenu && (
                        <button onClick={() => startEditProduct(p)} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-coffee-100 text-coffee-700 hover:bg-coffee-200">
                          Редагувати
                        </button>
                      )}
                      ...
                      {isManualMenu && (
                        <button onClick={() => deleteProduct(p)} className="px-3 py-1.5 rounded-lg text-xs font-medium bg-red-50 text-red-700 hover:bg-red-100">
                          Видалити
                        </button>
                      )}
                    </div>
          ...
          <div key={loc.id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm space-y-3">
            ...
            <div className="rounded-xl bg-gray-50 border border-gray-100 p-4 space-y-2 text-xs text-gray-600">
              ...
              {editingId === loc.id ? (
                ...
                <button onClick={() => startEditConfig(loc)} className="px-3 py-2 rounded-lg bg-coffee-100 text-coffee-700 text-xs font-medium hover:bg-coffee-200">
                  Редагувати POS
                </button>
              )}
            </div>
