    ...
    <div className="p-4 pb-24 h-full flex flex-col gap-3">
      ...
      <div className="flex-1 bg-white rounded-xl border border-gray-100 p-3 space-y-2 overflow-y-auto">
        {messages.map((m) => (
          <div key={m.id} className="border-b border-gray-50 pb-2">
            ...
            {m.isMine && (
              <button className="text-xs text-red-500 mt-1" onClick={() => onDelete(m.id)}>Видалити</button>
            )}
          </div>
        ))}
      </div>
      ...
      <form onSubmit={onSend} className="flex gap-2">
        <input
          ...
          maxLength={500}
        />
        <button disabled={loading} className="px-4 py-2 rounded-xl bg-coffee-600 text-white text-sm font-semibold disabled:opacity-60">
          Надіслати
        </button>
      </form>
    </div>
