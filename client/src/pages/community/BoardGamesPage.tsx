  ...
  return (
    <div className="p-4 pb-24 space-y-4">
      ...
      <form onSubmit={createMeetup} className="bg-white rounded-2xl border border-gray-100 p-4 space-y-2">
        ...
        <button className="w-full py-2 rounded-lg bg-coffee-600 text-white text-sm font-semibold">Створити</button>
      </form>
        ...
        {meetups.map((m) => (
          <div key={m.id} className="bg-white rounded-xl border border-gray-100 p-3">
            ...
            <div className="mt-2 flex gap-2">
              {!m.isJoined ? (
                <button onClick={async () => { await communityApi.joinMeetup(m.id); await load() }} className="px-3 py-1.5 rounded-lg bg-coffee-600 text-white text-xs">Приєднатись</button>
              ) : (
                <button onClick={async () => { await communityApi.leaveMeetup(m.id); await load() }} className="px-3 py-1.5 rounded-lg bg-gray-100 text-gray-700 text-xs">Покинути</button>
              )}
            </div>
          </div>
        ))}
