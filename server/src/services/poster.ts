...
function parsePrice(item: Record<string, unknown>): number {
  ...
  if (priceField && typeof priceField === 'object') {
    const vals = Object.values(priceField as Record<string, string>)
    if (vals.length > 0) {
      return parseFloat(String(vals[0])) / 100
    }
  }
...
export async function syncPosterMenu(locationSlug: string): Promise<{ synced: number; errors: string[] }> {
  ...
  const products: Record<string, unknown>[] = Array.isArray(data.response)
    ? data.response as Record<string, unknown>[]
    : Object.values(data.response as Record<string, unknown>) as Record<string, unknown>[]
  ...
  for (const item of products) {
    try {
      ...
      errors.push(String(item.product_id) + ': ' + msg)
    }
  }
...
export async function syncAllLocations(): Promise<void> {
  for (const loc of LOCATIONS) {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err)
      console.error('[Poster] Failed ' + loc.slug, msg)
    }
  }
}
